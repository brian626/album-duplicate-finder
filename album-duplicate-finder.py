...
def find_potential_duplicates(albums_with_line_numbers, similarity_threshold=0.85):
    """
    ...
    for line_num, original, normalized in normalized_albums:
        try:
            ...
            print(f"Warning: Line {line_num}: Entry doesn't follow 'Artist - Album' format: {original}")
            continue
    ...
    for artist, albums in artist_groups.items():
        ...
        for i in range(len(albums)):
            ...
            album_i = normalized_i.split(' - ', 1)[1] if ' - ' in normalized_i else normalized_i
            
            duplicates_for_i = [(line_num_i, original_i)]
            
            for j in range(i + 1, len(albums)):
                line_num_j, original_j, normalized_j = albums[j]
                album_j = normalized_j.split(' - ', 1)[1] if ' - ' in normalized_j else normalized_j
...
def main():
    parser = argparse.ArgumentParser(description='Find duplicate albums in a list.')
    parser.add_argument('input_file', help='Path to file containing album list')
    parser.add_argument('--threshold', type=float, default=0.85, 
                        help='Similarity threshold (0.0-1.0, higher is more strict)')
    ...
    potential_duplicates = find_potential_duplicates(albums_with_line_numbers, args.threshold)
    
    # Prepare output
    output_lines = []
    output_lines.append(f"Found {len(potential_duplicates)} potential duplicate groups:")
    output_lines.append("-" * 50)
    
    for i, group in enumerate(potential_duplicates, 1):
        output_lines.append(f"Group {i}:")
        for line_num, album in group:
            # Line numbers are already correct since we preserved them from the file
